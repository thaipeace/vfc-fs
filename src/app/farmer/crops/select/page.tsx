  ...
  return (
    <main className="fixed inset-0 z-[60] flex flex-col overflow-hidden bg-[#0C4A3F] font-sans">
      ...
      <div className="bg-white/10 backdrop-blur-md px-4 py-4 flex items-center justify-between border-b border-white/5">
        ...
        <h1 className="text-white font-bold text-lg flex-1 text-center truncate px-2">Thêm loại cây trồng</h1>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 gap-x-4 gap-y-6">
            {filteredCrops.map(crop => (
              ...
              >
                <div className={`relative w-10 h-10 rounded-full overflow-hidden transition-all ${selectedIds.includes(crop.id) ? 'ring-2 ring-[#FFD680]' : 'ring-1 ring-white/20'}`}>
                ...
                <span className="text-white text-[10px] font-bold text-center group-active:scale-95 transition-transform">{crop.name}</span>
              </button>
            ))}
          </div>
        )}
      ...
      <div className="fixed bottom-0 right-0 w-full h-1/2 pointer-events-none opacity-20">
         <Image src="/assets/images/bg.svg" alt="decor" fill className="object-cover object-bottom" />
      </div>
    </main>
  );
}
